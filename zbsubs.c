...
/* Subroutine */ int zbesh_(double *zr, double *zi, double *fnu,
  ...
  *nz = 0;
  ...
  goto L70;
L60:
  zuoik_(&znr, &zni, fnu, kode, &c__2, &nn, &cyr[1], &cyi[1], &nuf, &tol, &elim,
         &alim);
  if (nuf < 0) {
    goto L230;
  }
...
/* Subroutine */ int zbesi_(double *zr, double *zi, double *fnu,
...
/* Subroutine */ int zbesj_(double *zr, double *zi, double *fnu,
                            int *kode, int *n, double *cyr, double *cyi,
                            int *nz, int *ierr) {
  /* System generated locals */
  int i__1, i__2;
...
/* Subroutine */ int zbesk_(double *zr, double *zi, double *fnu,
                            int *kode, int *n, double *cyr, double *cyi,
                            int *nz, int *ierr) {
  /* System generated locals */
  int i__1, i__2;
...
/* Subroutine */ int zbesy_(double *zr, double *zi, double *fnu,
                            int *kode, int *n, double *cyr, double *cyi,
                            int *nz, double *cwrkr, double *cwrki,
                            int *ierr) {
  /* Initialized data */
...
/* Subroutine */ int zairy_(double *zr, double *zi, int *id, int *kode,
...
L100:
  ...
  zacai_(&ztar, &ztai, &fnu, kode, &mr, &c__1, cyr, cyi, &nn, &rl, &tol, &elim,
         &alim);
  if (nn < 0) {
    goto L280;
  }
...
/* Subroutine */ int zbiry_(double *zr, double *zi, int *id, int *kode,
...
/* Subroutine */ int zasyi_(double *zr, double *zi, double *fnu,
  ...
  double az;
  int nn;
  double p1i, s2i, p1r, s2r, cki, dki, fdn, arg, aez, arm, ckr, dkr, czi,
      ezi, sgn;
  int inu;
...
/* Subroutine */ int zacon_(double *zr, double *zi, double *fnu,
  ...
  *nz = 0;
  ...
  zni = -(*zi);
  nn = *n;
  zbinu_(&znr, &zni, fnu, kode, &nn, &yr[1], &yi[1], &nw, rl, fnul, tol, elim,
         alim);
  if (nw < 0) {
    goto L90;
  }
...
L110:
  ...
  nui = max(nui, 0);
  zbuni_(zr, zi, fnu, kode, &nn, &cyr[1], &cyi[1], &nw, &nui, &nlast, fnul, tol,
         elim, alim);
  if (nw < 0) {
    goto L130;
  }
...
/* Subroutine */ int
zunhj_(double *zr, double *zi, double *fnu, int *ipmtr,
...
/* Subroutine */ int zunk1_(double *zr, double *zi, double *fnu,
  ...
  double fn;
  int il, kk, nw;
  double c1i, c2i, c2m, c1r, c2r, s1i, s2i, rs1, s1r, s2r, ang, asc, cki,
      fnf;
  int ifn;
  double ckr;
  int iuf;
...
L75:
  ...
  initd = 0;
  zunik_(&zrr, &zri, &fn, &c__2, &ipard, tol, &initd, &phidr, &phidi, &zet1dr,
...
L170:
  ...
  L175:
    ...
    initd = 0;
  L180:
    zunik_(&zrr, &zri, &fn, &c__1, &c__0, tol, &initd, &phidr, &phidi, &zet1dr,
...
L85:
  ...
  zunhj_(&znr, &zni, &fn, &ipard, tol, &phidr, &phidi, &argdr, &argdi, &zet1dr,
...
L190:
  ...
  L175:
    ...
    zunhj_(&znr, &zni, &fn, &c__0, tol, &phidr, &phidi, &argdr, &argdi, &zet1dr,
           &zet1di, &zet2dr, &zet2di, &asumdr, &asumdi, &bsumdr, &bsumdi);
  L210:
    if (*kode == 1) {
      goto L220;
...
/* Subroutine */ int zbuni_(double *zr, double *zi, double *fnu,
                            int *kode, int *n, double *yr, double *yi,
                            int *nz, int *nui, int *nlast, double *fnul,
                            double *tol, double *elim,
                            double *alim) {
  /* System generated locals */
  int i__1;
...
/* Subroutine */ int zuni1_(double *zr, double *zi, double *fnu,
  ...
  *nlast = 0;
  ...
  fn = max(*fnu, 1.);
  init = 0;
  zunik_(zr, zi, &fn, &c__1, &c__1, tol, &init, &phir, &phii, &zeta1r, &zeta1i,
...
L40:
  ...
  for (i__ = 1; i__ <= i__1; ++i__) {
    fn = *fnu + (double)((float)(nd - i__));
    zunhj_(&znr, &zni, &fn, &c__0, tol, &phir, &phii, &argr, &argi, &zeta1r,
           &zeta1i, &zeta2r, &zeta2i, &asumr, &asumi, &bsumr, &bsumi);
    if (*kode == 1) {
      goto L50;
    }
